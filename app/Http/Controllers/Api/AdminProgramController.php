...
class AdminProgramController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'work_mode' => $workMode,
            'programs' => $q->get()->map(fn (Program $p) => $this->programPayload($p, $request->user(), $workMode))->values(),
        ]);
    }
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json([
            'work_mode' => $this->programWorkMode($request->user()),
            'program' => $this->programPayload($program, $request->user()),
        ]);
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        return response()->json(['program' => $this->programPayload($program->load(['project:id,name', 'period:id,name,status', 'managingUnit:id,name']), $request->user())], 201);
    }
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        abort_if(
            $program->isCommunityEvent() && ! $this->permissionResolver->hasGlobalScope($request->user(), 'programs.update'),
            403,
    ...
    public function storeCommunityEvent(Request $request): JsonResponse
    {
        $v = $this->validatedCommunityEventData($request, true);
        $unit = $this->communityProgramAccess->managingUnitFor(
            $request->user(),
        ...
        $program = Program::query()->create($v + [
            ...
            'feedback_form_template_id' => null,
            'is_public' => false,
            'is_featured' => false,
        ...
        return response()->json([
            'program' => $this->programPayload(
                $program->load(['project:id,name', 'period:id,name,status', 'managingUnit:id,name']),
                $request->user()
            ),
        ], 201);
    }

    public function updateCommunityEvent(Request $request, int $id): JsonResponse
    {
        $program = Program::query()->with(['project:id,name', 'period:id,name,status', 'managingUnit:id,name'])->findOrFail($id);
        abort_unless(
            $this->communityProgramAccess->canAccess($request->user(), 'programs.community_event.update', $program),
            403,
            'Bu ortak etkinligi guncelleme yetkiniz bulunmuyor.'
        );
        $v = $this->validatedCommunityEventData($request, false, $program);
        if (array_key_exists('project_id', $v) && (int) $v['project_id'] !== (int) $program->project_id) {
            $targetUnit = $this->communityProgramAccess->managingUnitFor(
                $request->user(),
                'programs.community_event.update',
                (int) $v['project_id']
            );
            abort_unless(
                $targetUnit && (int) $targetUnit->id === (int) $program->managing_unit_id,
                403,
                'Ortak etkinlik yalniz ayni birimin sorumlu oldugu projeye tasinabilir.'
            );
        }
        $periodId = array_key_exists('period_id', $v) ? (int) $v['period_id'] : (int) $program->period_id;
        $this->assertPeriodWritable($request, $periodId);
        ...
        return response()->json([
            'program' => $this->programPayload(
                $program->fresh(['project:id,name', 'period:id,name,status', 'managingUnit:id,name']),
                $request->user()
            ),
        ]);
    }
    ...
    {
        $program = Program::query()->with(['project:id,name', 'period:id,name,status', 'managingUnit:id,name'])->findOrFail($id);
        $this->abortUnlessProjectAllowed($request, 'programs.logistics.update', (int) $program->project_id);
        $this->assertPeriodWritable($request, $program->period_id);
        $v = $request->validate([
            'location' => ['sometimes', 'nullable', 'string', 'max:255'],
            'location_place_name' => ['sometimes', 'nullable', 'string', 'max:255'],
            'location_place_address' => ['sometimes', 'nullable', 'string', 'max:500'],
            'location_place_id' => ['sometimes', 'nullable', 'string', 'max:255'],
            'location_place_provider' => ['sometimes', 'nullable', 'string', 'max:50'],
            'latitude' => ['sometimes', 'nullable', 'numeric', 'between:-90,90'],
            'longitude' => ['sometimes', 'nullable', 'numeric', 'between:-180,180'],
            'radius_meters' => ['sometimes', 'nullable', 'integer', 'min:10', 'max:5000'],
            'guest_info' => ['sometimes', 'nullable', 'array'],
        ]);
        abort_if($v === [], 422, 'Guncellenecek lojistik alani bulunamadi.');
        $program->update($v);
    ...
    public function complete(Request $request, int $id): JsonResponse
    {
        ...
        abort_if(
            $program->isCommunityEvent() && ! $this->permissionResolver->hasGlobalScope($request->user(), 'programs.complete'),
            403,
            'Ortak etkinlik cekirdek program tamamlama akisindan tamamlanamaz.'
        );
        $this->assertPeriodResolvable($request, $program->period_id);
        ...
        return response()->json(['message' => 'Program tamamlandi.', 'program' => $this->programPayload($program->fresh(['project:id,name', 'period:id,name,status', 'managingUnit:id,name']), $request->user()), 'deducted_participant_count' => $deducted]);
    }
    ...
    public function markManualAttendance(Request $request, int $id, int $participantId): JsonResponse
    {
        ...
        $attendance = DB::transaction(function () use ($program, $participant, $v, $request) {
            ...
            if ($this->participantHasCreditImpact($participant)) {
                $this->creditService->reconcileCompletedProgramAttendance($program, $participant, (bool) $v['is_valid'], $request->user()->id);
            }
        ...
        return response()->json(['message' => $attendance->is_valid ? 'Yoklama katildi olarak isaretlendi.' : 'Yoklama gelmedi olarak isaretlendi.', 'attendance' => [
            'id' => (int) $attendance->id,
            'program_id' => (int) $attendance->program_id,
    ...
    {
        $program = Program::query()->with(['project:id,name', 'period:id,name,status'])->findOrFail($id);
        $attendancePermission = $this->authorizeAttendance($request, $program, 'export');
        $minimal = $attendancePermission === 'programs.community_event.attendance.export';
        $records = $this->attendanceRecords($program, $minimal);
        if ($minimal) {
            $rows = $records->map(fn (array $r) => [$r['id'] ?? '-', $r['student'], $r['attendance_status'] ?? '-', $r['method'] ?? '-', $r['recorded_at'] ?? '-'])->all();

            return AdminExportResponder::download($request->string('format')->toString() ?: 'csv', 'program_'.$program->id.'_yoklama_'.now()->format('Ymd_His'), 'Program Yoklama Detaylari', ['Yoklama ID', 'Katilimci', 'Durum', 'Yontem', 'Kayit Zamani'], $rows);
        }
        $rows = $records->map(fn (array $r) => [$r['id'] ?? '-', $r['student'], $r['email'] ?? '-', $r['attendance_status'] ?? '-', $r['method'] ?? '-', $r['feedback_submitted'] ? 'evet' : 'hayir', $r['credit_deducted'] ? 'evet' : 'hayir', $r['credit_restored'] ? 'evet' : 'hayir', $r['recorded_at'] ?? '-'])->all();
    ...
    public function feedbackStats(Request $request, int $id): JsonResponse
    {
        ...
        foreach ($ratingQuestions as $q) {
            ...
            $dist = [];
            for ($s = (int) ($q['min'] ?? 1); $s <= (int) ($q['max'] ?? 5); $s++) {
                $dist[(string) $s] = $values->filter(fn ($v) => (float) $v === (float) $s)->count();
            }
        ...
        $responses = $feedbacks->map(function (Feedback $f) use ($questions, $commentQuestions) {
            ...
            $textAnswers = collect($commentQuestions)->map(function ($q) use ($f) {
                $value = trim((string) ($f->responses[$q['id']] ?? ''));

                return $value === '' ? null : ['question_id' => $q['id'], 'question' => $q['label'], 'answer' => $value];
            })->filter()->values()->all();
    ...
    public function photos(Request $request, int $id): JsonResponse
    {
        ...
        abort_unless(
            $this->canViewProgramMedia($request->user(), $program),
            403,
            'Bu programin galerisini goruntuleme yetkiniz bulunmuyor.'
        );
    ...
    private function validatedProgramData(Request $request, bool $creating, ?Program $program = null): array
    {
        ...
        if (isset($v['target_audience'])) {
            $v['target_audience'] = collect($v['target_audience'])->unique()->values()->all();
        } elseif ($creating) {
            $v['target_audience'] = ['student'];
        }
        ...
        $targetProjectId = array_key_exists('project_id', $v) ? (int) $v['project_id'] : (int) $program?->project_id;
        $targetPeriodId = array_key_exists('period_id', $v) ? (int) $v['period_id'] : (int) $program?->period_id;
        if ($targetProjectId && $targetPeriodId) {
            $belongs = Period::query()->whereKey($targetPeriodId)->where('project_id', $targetProjectId)->exists();
            if (! $belongs) {
                throw ValidationException::withMessages(['period_id' => ['Secilen donem bu projeye ait degil.']]);
            }
        }
    ...
    private function validatedCommunityEventData(Request $request, bool $creating, ?Program $program = null): array
    {
        ...
        $rules = [
            ...
            'target_audience.*' => ['string', Rule::in(['student', 'alumni'])],
            'status' => ['sometimes', Rule::in(['scheduled', 'active', 'cancelled'])],
        ];
        $v = $request->validate($rules);
        if (isset($v['target_audience'])) {
            $v['target_audience'] = collect($v['target_audience'])->unique()->values()->all();
        ...
        if ($creating) {
            $v['radius_meters'] = $v['radius_meters'] ?? 100;
    ...
    private function programPayload(Program $p, ?User $user = null, ?string $workMode = null): array
    {
        ...
        $canCoreDetails = $workMode === 'core';
        $capabilities = [
            'update_core' => ! $p->isCommunityEvent() && $this->permissionResolver->canAccessProject($user, 'programs.update', (int) $p->project_id),
            'update_community_event' => $this->communityProgramAccess->canAccess($user, 'programs.community_event.update', $p),
            'update_logistics' => $this->permissionResolver->canAccessProject($user, 'programs.logistics.update', (int) $p->project_id),
            'view_attendance' => $this->canAccessAttendance($user, $p, 'view'),
            'manage_attendance' => $this->canAccessAttendance($user, $p, 'manage'),
            'export_attendance' => $this->canAccessAttendance($user, $p, 'export'),
            'manage_media' => $this->permissionResolver->canAccessProject($user, 'programs.media.upload', (int) $p->project_id),
            'view_media' => $this->canViewProgramMedia($user, $p),
            'complete' => ! $p->isCommunityEvent() && $this->permissionResolver->canAccessProject($user, 'programs.complete', (int) $p->project_id),
            'manage_qr' => $this->permissionResolver->canAccessProject($user, 'programs.qr.manage', (int) $p->project_id),
        ];
        $base['work_mode'] = $workMode;
    ...
    private function attendanceRecords(Program $program, bool $minimal = false)
    {
        ...
        return $this->programParticipantsQuery($program)->orderBy('id')->get()->map(function (Participant $p) use ($att, $logsByUser, $tokens, $program, $minimal) {
            ...
            $base = ['id' => $a?->id, 'participant_id' => $p->id, 'student' => $p->user ? trim($p->user->name.' '.$p->user->surname) : 'Silinmis kullanici', 'role' => $p->user?->role, 'method' => $a?->method, 'is_valid' => (bool) ($a?->is_valid ?? false), 'attendance_status' => $a?->is_valid ? 'present' : 'absent', 'recorded_at' => optional($a?->created_at)?->toIso8601String()];
            if ($minimal) {
                return $base;
            }
    ...
    private function programViewPermission(Request $request): string
    {
        if ($this->permissionResolver->hasPermission($request->user(), 'programs.view')) {
            return 'programs.view';
        }
        if ($this->permissionResolver->hasPermission($request->user(), 'programs.community_event.view')) {
            return 'programs.community_event.view';
        }
        $this->abortUnlessAllowed($request, 'programs.logistics.view');

        return 'programs.logistics.view';
    }

    private function authorizeProgramView(Request $request, Program $program): string
    {
        if ($this->permissionResolver->canAccessProject($request->user(), 'programs.view', (int) $program->project_id)) {
            $mode = $this->programWorkMode($request->user());
            if (! $program->isCommunityEvent() || $mode === 'media' || $this->permissionResolver->hasGlobalScope($request->user(), 'programs.view')) {
                return 'programs.view';
            }
        }
        if ($this->communityProgramAccess->canAccess($request->user(), 'programs.community_event.view', $program)) {
            return 'programs.community_event.view';
        }
        if ($this->permissionResolver->canAccessProject($request->user(), 'programs.logistics.view', (int) $program->project_id)) {
            return 'programs.logistics.view';
        }
        abort(403, 'Bu programi goruntuleme yetkiniz bulunmuyor.');
    }

    private function programWorkMode(User $user): string
    {
        ...
        if ($this->permissionResolver->hasPermission($user, 'programs.view')) {
            ...
            if (! $hasCoreOperation && $this->permissionResolver->hasPermission($user, 'programs.media.upload')) {
                return 'media';
            }
    ...
    private function canViewProgramMedia(User $user, Program $program): bool
    {
        if ($this->permissionResolver->canAccessProject($user, 'programs.media.upload', (int) $program->project_id)) {
            return true;
        }

        if ($this->permissionResolver->canAccessProject($user, 'programs.view', (int) $program->project_id)) {
            return ! $program->isCommunityEvent()
                || $this->permissionResolver->hasGlobalScope($user, 'programs.view');
        }

        return $this->communityProgramAccess->canAccess($user, 'programs.community_event.view', $program);
    }

    private function canAccessAttendance(User $user, Program $program, string $operation): bool
    {
        $corePermission = 'programs.attendance.'.$operation;
        if ($this->permissionResolver->canAccessProject($user, $corePermission, (int) $program->project_id)) {
            return true;
        }
        $communityPermission = 'programs.community_event.attendance.'.$operation;

        return $this->communityProgramAccess->canAccess($user, $communityPermission, $program);
    }

    private function authorizeAttendance(Request $request, Program $program, string $operation): string
    {
        $corePermission = 'programs.attendance.'.$operation;
        if ($this->permissionResolver->canAccessProject($request->user(), $corePermission, (int) $program->project_id)) {
            $request->attributes->set('audit.permission_checked', $corePermission);

            return $corePermission;
        }
        $communityPermission = 'programs.community_event.attendance.'.$operation;
        abort_unless(
            $this->communityProgramAccess->canAccess($request->user(), $communityPermission, $program),
            403,
            'Bu programin yoklamasi icin yetkiniz bulunmuyor.'
        );
        $request->attributes->set('audit.permission_checked', $communityPermission);
    ...
    private function buildFeedbackSummary(Request $request): array
    {
        ...
        $this->applyProjectPeriodContext($q, $ctx);
        $programs = $q->get();
        $byProgram = Feedback::query()->whereIn('program_id', $programs->pluck('id'))->orderByDesc('submitted_at')->get()->groupBy('program_id');
        $questionStats = [];
        $programRows = [];
        $recent = [];
        $allRatings = collect();
        ...
        foreach ($programs as $program) {
            $feedbacks = $byProgram->get($program->id, collect());
            $questions = FeedbackFormResolver::forProgram($program);
            $rating = FeedbackFormResolver::ratingQuestions($questions);
            $choices = FeedbackFormResolver::choiceQuestions($questions);
            $comments = FeedbackFormResolver::commentQuestions($questions);
            $programRatings = collect();
            $programWithComment = 0;
            foreach ($rating as $question) {
                $key = $question['id'];
                $values = $feedbacks->map(fn (Feedback $f) => $f->responses[$key] ?? null)->filter(fn ($v) => is_numeric($v))->map(fn ($v) => (float) $v)->values();
                $questionStats[$key] ??= ['label' => $question['label'], 'type' => 'rating', 'count' => 0, 'sum' => 0, 'distribution' => []];
                for ($s = (int) ($question['min'] ?? 1); $s <= (int) ($question['max'] ?? 5); $s++) {
                    $questionStats[$key]['distribution'][(string) $s] ??= 0;
                    $questionStats[$key]['distribution'][(string) $s] += $values->filter(fn ($v) => (float) $v === (float) $s)->count();
                } $questionStats[$key]['count'] += $values->count();
                $questionStats[$key]['sum'] += $values->sum();
                $programRatings = $programRatings->merge($values);
                $allRatings = $allRatings->merge($values);
            }
            foreach ($choices as $question) {
                $key = $question['id'];
                $values = $feedbacks->map(fn (Feedback $f) => $f->responses[$key] ?? null)->filter(fn ($v) => $v !== null && $v !== '')->values();
                $questionStats[$key] ??= ['label' => $question['label'], 'type' => 'choice', 'count' => 0, 'distribution' => collect($question['options'] ?? [])->mapWithKeys(fn ($o) => [$o => 0])->all()];
                foreach (($question['options'] ?? []) as $o) {
                    $questionStats[$key]['distribution'][$o] ??= 0;
                    $questionStats[$key]['distribution'][$o] += $values->filter(fn ($v) => (string) $v === (string) $o)->count();
                } $questionStats[$key]['count'] += $values->count();
            }
            foreach ($feedbacks as $f) {
                $has = false;
                foreach ($comments as $question) {
                    $comment = trim((string) ($f->responses[$question['id']] ?? ''));
                    if ($comment === '') {
                        continue;
                    } $has = true;
                    $recent[] = ['program_id' => $program->id, 'program_title' => $program->title, 'project' => $program->project?->name, 'question' => $question['label'], 'comment' => $comment, 'submitted_at' => optional($f->submitted_at)?->toIso8601String()];
                } if ($has) {
                    $programWithComment++;
                    $withComment++;
                }
        ...
        $normalized = collect($questionStats)->map(function (array $s) {
            if (($s['type'] ?? null) === 'rating') {
                $c = (int) ($s['count'] ?? 0);
                $s['average'] = $c > 0 ? round(((float) $s['sum']) / $c, 2) : null;
                unset($s['sum']);
            }

return $s;
        })->all();
        $rows = collect($programRows);
        $break = function (string $key, string $fallback) use ($rows): array {
            return $rows->groupBy(fn (array $r) => $r[$key] ?: $fallback)->map(function ($items, string $name) {
                $count = (int) $items->sum('rating_count');
                $avg = $count > 0 ? round($items->sum(fn (array $r) => (($r['overall_average'] ?? 0) * (int) ($r['rating_count'] ?? 0))) / $count,2) : null;

                return ['name' => $name, 'program_count' => $items->count(), 'feedback_count' => (int) $items->sum('feedback_count'), 'with_comment' => (int) $items->sum('with_comment'), 'overall_average' => $avg];
            })->sortByDesc('feedback_count')->values()->all();
        };
